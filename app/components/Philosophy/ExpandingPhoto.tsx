...
const ExpandingPhoto: React.FC = () => {
  ...
  const acceleratedProgress = useTransform(currentProgress, [0, accelerationFactor], [0, 1]);


  const [expanded, setExpanded] = useState(false);
  useEffect(() => {
    const unsubscribe = acceleratedProgress.on("change", (value) => {
      setExpanded(value >= 1);
    });
    return () => unsubscribe();
  }, [acceleratedProgress]);
      ...
      if (current < accelerationFactor) {
        ...
        if (newValue < accelerationFactor) {
          ...
          if (accumulatedDeltaRef.current > scrollStartThreshold) {
            ...
            const factor = targetProgress.get() < accelerationFactor ? 0.01 : pageScrollFactor;
  ...
  return (
    <div className="relative w-full h-screen pt-12" style={{ overflow: 'hidden' }} onWheel={handleWheel}>
      <motion.img
